...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (typeof attending !== 'boolean' || !Number.isInteger(passes) || passes < 0) {
      return json({ message: 'Revisa los datos de tu confirmación.' }, 400);
    }
    // La función SQL aplica la regla final: recepción requiere al menos un pase;
    // una invitación de solo misa confirma correctamente con cero pases.
    if (!attending && passes !== 0) return json({ message: 'La cantidad de pases no es válida.' }, 400);

    const database = getWeddingDatabase();
    const { data, error } = await database.rpc('confirm_wedding_invitation', {
      p_public_token: token,
      p_attending: attending,
      p_confirmed_passes: passes,
      p_note: note || null
    });

    if (error) {
      if (error.message.includes('PASS_LIMIT')) return json({ message: 'La cantidad seleccionada supera los pases asignados.' }, 400);
